    ...
    if form.validate_on_submit():
        try:
            if User.authenticate(g.user.username, form.password.data):
                ...
                flash("Changes made sucessfully", 'success')
                return redirect(f"/users/{g.user.id}")
            else:
                form.username.errors = ['Invalid password']
                flash("Authentication error. Password incorrect", 'danger')

        except IntegrityError:
            flash("Error. Redirecting to homepage", 'danger')
...
def like_message(message_id):
    ...
    if not g.user:
        flash("Please login to like a message", "danger")
        return redirect('/')
...
def dislike_message(message_id):
    ...
    if not g.user:
        flash("Please login to dislike a message", "danger")
        return redirect('/')
...
def homepage():
    ...
    if g.user:
        ...
        ids = [l.message_id for l in Likes.query.filter_by(user_id=g.user.id)]

        return render_template('home.html', messages=messages, ids=ids)
